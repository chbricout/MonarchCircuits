...
if __name__ == "__main__":
    ...
    # Model args
    parser.add_argument(
        "-ly", "--layer_type", type=str, default="monarch", help="layer type to use"
    )
    parser.add_argument(
        "-hs",
        "--hidden_size",
        type=int,
        default=1024,
        help="hidden size of model; must be 2^{2n} for positive integer n",
    )
    # Training args
    ...
    parser.add_argument(
        ...
        type=str,
        default="imagenet32",
        help="dataset (either imagenet32 or imagenet64)",
    )
    # Dataset args
    parser.add_argument(
        "-ll",
        "--lossless",
        action="store_true",
        help="whether to train on lossless (YCoCg-R) or lossy (YCoCg) transformed data",
    )
    parser.add_argument(
        "-ps",
        "--patch_size",
        type=int,
        default=8,
        help="image patch sizes for training",
    )
    # Misc args
    parser.add_argument(
        "-sloc", "--save_location", type=str, default="model.jpc", help="save location"
    )
    parser.add_argument(
        "-lloc", "--load_location", type=str, default=None, help="load location"
    )
    parser.add_argument(
        "-dv",
        "--devices",
        type=str,
        default="0",
        help="devices to use, comma separated",
    )
