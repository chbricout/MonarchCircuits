...
def BayesianTreeToHiddenRegionGraphGeneral(
    tree: nx.Graph,
    root,
    num_latents: int,
    ...
    for v in node_seq:
        ...
        if len(chs) == 0:
            ...
            if leaf_idx not in leaf_nodes:
                ...
                leaf_r = leaf_nodes[leaf_idx].duplicate(
                    v, tie_params=homogeneous_inputs
                )
            ...
            if v == root:
                ...
                r, ns_sum = layer_fn(
                    *ch_regions,
                    num_node_blocks=num_node_blocks,
                    block_size=block_size,
                    permute_block_size=permute_block_size,
                    existing_nodes=ns_sum,
                    homogeneous=False,
...
def HCLTGeneral(
    ...
    # Custom code to dump graph
    clt = nx.bfs_tree(T, root)
    n = T.number_of_nodes()
    predecessors = -1 * np.ones(n, dtype=int)
    for parent, child in clt.edges():
        predecessors[child] = parent
    predecessors.tofile("clt_tree.csv", sep=",")
    # End of Custom code

    if block_size is not None:
        assert num_latents % block_size == 0, (
            "num_latents must be divisible by sparse_block_size"
        )
    else:  # assume monarch
        assert (num_latents**0.5) == int(num_latents**0.5), (
            "num_latents must be a square number"
        )
