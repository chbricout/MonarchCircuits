...
def create_dense_layer(
    ...
    **kwargs,
    ...
    for i in range(num_layers):
        ...
        if i < num_layers - 1:
            np = multiply(
                ns, edge_ids=torch.arange(0, num_node_blocks)[:, None]
            )  # passthrough layer

    return ns, sum_nodes


def _create_monarch_layer(
    ns, num_node_blocks, block_size, permute_block_size, existing_node, homogeneous
):
    print("creating monarch layer!")
    permuted_edges = (
        torch.arange(0, block_size * num_node_blocks)
        .reshape(num_node_blocks * block_size // permute_block_size, permute_block_size)
        .permute(1, 0)
        .reshape(block_size * num_node_blocks)[:, None]
    )
    np = multiply(ns, edge_ids=permuted_edges, sparse_edges=True)

    edge_ids = torch.arange(0, num_node_blocks)[None, :].repeat(
        2, 1
    )  # Tensor([[0, 1, ...], [0, 1, ...]])
...
def create_monarch_layers(
    *input_nodes,
    num_node_blocks,
    block_size,
    homogeneous,
    existing_nodes=None,
    permute_block_size=None,
    num_layers=3,
    **kwargs,
):
    ...
    if permute_block_size is None:
        ...
        assert permute_block_size % block_size == 0, (
            "permute_block_size must be divisible by block_size"
        )
    ...
    for layer_num in range(2, num_layers + 1):
        ns = _create_monarch_layer(
            ns,
            num_node_blocks,
            block_size,
            permute_block_size,
            existing_nodes.get(f"ns{layer_num}", None)
