...
from pc_mini_em.src.utils import (
    instantiate_from_config,
    collect_data_from_dsets,
    ProgressBar,
)
...
def load_data(dataset, patch_size, lossless, batch_size):
    ...
    if dataset == "imagenet32":
        ...
        data_config["params"]["validation"]["target"] = (
            "pc_mini_em.src.data.ImageNet64Validation"
        )
    if batch_size > 0:
    ...
    if (dataset == "imagenet32" and patch_size < 16) or (dataset == "imagenet64"):
        for i, transform in enumerate(
            data_config["params"]["train"]["params"]["transform_fns"]
        ):
            if "Patchify" in transform["target"]:
                data_config["params"]["train"]["params"]["transform_fns"][i]["params"][
                    "patch_size"
                ] = patch_size
                data_config["params"]["train"]["params"]["transform_fns"][i][
                    "target"
                ] = "pc_mini_em.src.transforms.Patchify"
        for i, transform in enumerate(
            data_config["params"]["validation"]["params"]["transform_fns"]
        ):
            if "Patchify" in transform["target"]:
                print("patchified")
                data_config["params"]["validation"]["params"]["transform_fns"][i][
                    "params"
...
def construct_model(dsets, lossless, layer_type, hidden_size):
    model_config = (
        f"pc_mini_em/configs/model/hclt_256{'_lossless' if lossless else ''}.yaml"
    )
    ...
    for k, v in model_config["params"].items():
        if isinstance(v, str) and v.startswith("__train_data__:"):
            ...
            data = collect_data_from_dsets(
                dsets, num_samples=num_samples, split="train"
            )
    ...
    if layer_type == "dense":
        ...
        model_config["params"]["block_size"] = int(
            model_config["params"]["num_latents"] ** 0.5
        )
    match layer_type:
        case "dense":
...
def construct_and_train_pc(rank, world_size, args):
    ...
    dsets, train_loader, val_loader = load_data(
        args.dataset, args.patch_size, args.lossless, args.batch_size
    )
    ...
    if rank == 0:
        if args.load_location is not None:
            ...
            ns = construct_model(
                dsets, args.lossless, args.layer_type, args.hidden_size
            )
    ...
    if rank == 0:
        progress_bar = ProgressBar(
            tot_epochs, len(train_loader), ["LL"], cumulate_statistics=True
        )
    ...
    for epoch in range(tot_epochs):
        ...
        for x in train_loader:
            ...
            with torch.cuda.device(f"cuda:{device_list[rank]}"):
                ...
                train_step_time.append(start.elpased_time(end) * 1e-3)
                gc.enable()
                gc.collect()
            ...
            if step_count >= niters_per_update:
                ...
                dist.barrier()
                current_time = (
                    epoch + update_count * sizes[era] / len(train_loader.dataset)
                ) / tot_epochs
                # Cosine LR decay
        ...
        for x in val_loader:
            ...
            with torch.cuda.device(f"cuda:{device_list[rank]}"):
                ...
                val_step_time.append(start.elpased_time(end) * 1e-3)
                gc.enable()
                gc.collect()
    ...
    if rank == 0:
        if args.save_location is not None:
            ...
            name = args.save_location.split(".")[0]
            pd.DataFrame(
                {
                    "val_ll": [val_ll],
                    "train_mem_avg": [torch.tensor([train_mems]).float().mean().item()],
                    "train_step_time": [
                        torch.tensor([train_step_time]).float().mean().item()
                    ],
                    "val_mem_avg": [torch.tensor([val_mems]).float().mean().item()],
                    "val_step_time": [
                        torch.tensor([val_step_time]).float().mean().item()
                    ],
                }
            ).to_csv(f"{name}.csv")
